...
/** Detail returned by `GET /ability/{name}` — used for the effect text on the About tab. */
data class AbilityDetailDto(
  val id: Int,
  val name: String,
  @SerializedName("effect_entries") val effectEntries: List<AbilityEffectEntryDto> = emptyList()
)
...
/** One entry of `pokemon-species.varieties` — an alternate form (default, Mega, etc.). */
